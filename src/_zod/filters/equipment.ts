...
const CONJUNCTION_TYPES = ["and", "or"] as const;
const EQUIPMENT_CATEGORIES = ["itemTypes", "rarity", "armorTypes", "weaponTypes"] as const;

function zTraitData<T extends string>() {
    return z.object({
        ...
        selected: z
            .array(
                z.object({
                    ...
                    value: z.string().trim() as unknown as z.ZodCustom<T, T>,
                }),
            )
            .default([]),
    });
...
function zLevelData(max: number) {
    return z.object({
