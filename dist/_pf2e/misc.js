...
function isControlDown(event) {
    return foundry.helpers.interaction.KeyboardManager.CONTROL_KEY_STRING === "⌘" ? event.metaKey : event.ctrlKey;
}
/**
...
function localizer(prefix) {
    return (...[suffix, formatArgs]) => formatArgs ? game.i18n.format(`${prefix}.${suffix}`, formatArgs) : game.i18n.localize(`${prefix}.${suffix}`);
}
